...
export function createGrepTool(cwd: string) {
    return tool({
        description:
        "Search file contents using a regex pattern. Returns matching lines with file path and line numbers. Skips hidden directories, node_modules, and binary files.",
        ...
        execute: async ({ pattern, path, includes }) => {
            ...
            return {
                matches,
                ...(truncated? {truncated: true, totaMatches: lines.length}: {})
            }
