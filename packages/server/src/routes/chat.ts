...
import{Mode, MessageStatus, Role} from "@owlcode/database/enums"
import { chatStreamEventSchema, type ChatStreamEvent } from "@owlcode/shared";
...
const app = new Hono()
  ...
  .post("/:sessionId/resume" , async (c) => {
    ...
    if(!isSupportedChatModel(lastMessage.model)) {
        return c.json({error: `Session user unsupported model: ${lastMessage.model}`},409)
    }
